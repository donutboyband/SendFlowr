...
if ! curl -s http://localhost:${API_PORT}/health > /dev/null 2>&1; then
    echo "⚠️  Inference API is not running on port ${API_PORT}!"
    echo ""
    ...
    python -m uvicorn main:app --reload --port ${API_PORT} > /dev/null 2>&1 &
    API_PID=$!
...
echo "✅ Inference API is running at http://localhost:${API_PORT}"
echo ""
...
echo ""
echo "✅ Inference pipeline test complete!"
echo ""
echo "🎯 Key Features:"
echo "  • Minute-level precision (10,080 slots)"
...
echo ""
echo "📖 See: docs/MIGRATION.md for architectural details"
echo ""
